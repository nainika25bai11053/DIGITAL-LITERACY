# DIGITAL-LITERACY
# 📘 Digital Literacy Project

##  Student Details

* **Name:** nainika kumari
* **Branch:** B.Tech CSE
* **Year:** 1st Year
* **College:** VIT UNIVERSITY
* **Registration Number:** 25BAI11O53

---

## 📌 Project Overview

This project is a comprehensive **Digital Literacy Portfolio** covering essential digital skills such as online safety, professional communication, coding practice, collaboration tools, and cybercrime awareness.

The repository includes multiple tasks demonstrating practical knowledge of digital tools and responsible internet usage.

---

## 📂 Project Structure

```
📁 Digital-Literacy-Portfolio
 ├── README.mD
 ├── Task-1 (Digital Literacy Awareness)
 ├── Task-2 (Digital Portfolio)
 ├── Task-3 (Coding & Collaboration)
 ├── Task-4 (Email & Etiquette)
 ├── Task-5 (Cyber Crime Awareness)
```

---

##  Task 1: Digital Literacy Awareness

This task explains the concept of digital literacy and safe internet practices.

### 🔹 Key Topics:

* Meaning of digital literacy
* Safe internet practices (strong passwords, phishing awareness)
* Useful tools:

  * Google Docs
  * Microsoft Excel
  * Zoom
  * Microsoft Word
  * LinkedIn
* Email etiquette basics

---

##  Task 2: Digital Portfolio

This task focuses on building a professional online presence.

### 🔹 Platforms Covered:

* **LinkedIn** – Professional profile creation
* **Kaggle** – Data science and coding profile
* **GitHub** – Project repository and version control

### 🔹 Outcome:

* Created and maintained professional profiles
* Learned how to showcase skills online

---

## Task 3: Coding & Collaboration

### 🔹 Part A: HackerRank

* Practiced coding problems
* Improved programming skills

### 🔹 Part B: Google Workspace

* **Google Form:** Created a digital literacy quiz
* **Google Sheets:** Collected and analyzed responses

---

## Task 4: Email & Etiquette

## Part A:
### 🔹 Email 1: Assignment Extension Request

A formal email requesting extra time due to academic workload.

### 🔹 Email 2: Internship Application

A professional email expressing interest in a summer internship.

## PART B:

  ### Social media Do's and Don'ts checklist 
  
### 🔹 Key Learnings:

* Proper subject line
* Formal tone
* Clear and respectful communication
* what to do and what not to while using social media

---

##  Task 5: Cyber Crime Awareness

### 🔹 Case Study: UPI Payment Fraud

* Fraudsters trick users into entering UPI PIN
* Common methods:

  * Fake calls/messages
  * Payment request scams
* **Impacts:**

  * Financial loss
  * Mental stress

### 🔹 Prevention Tips:

* Never share OTP or UPI PIN
* Avoid unknown links
* Verify payment requests

---

## Tools & Technologies Used

* Google Docs
* Google Forms
* Google Sheets
* GitHub
* LinkedIn
* Kaggle
* HackerRank
* canva
* Notes
---

## Learning Outcomes

* Improved digital awareness
* Developed professional online presence
* Learned safe internet practices
* Gained basic coding and collaboration experience
* Understood cybercrime risks and prevention
* Gained knowledge about how to make google forms and how to use all the digital tools.

---

## 🔗 Links

* 🔗 LinkedIn: https://github.com/nainika25bai11053
* 🔗 GitHub: www.linkedin.com/in/nainika-kumari-8171953ba
* 🔗 Kaggle: https://www.kaggle.com/nainika25bai11053
* 🔗 HackerRank: https://www.hackerrank.com/profile/nainika3225
* google form : https://docs.google.com/forms/d/e/1FAIpQLSfF4jvTiRVRHeowZm_OPX7j7NE2eF2Xxp8XvqaTWfKm8nuOhA/viewform?usp=publish-editor

---

## 📢 Conclusion

This project helped in building essential digital skills required in today’s world, including communication, collaboration, cybersecurity awareness, and professional development.

---
